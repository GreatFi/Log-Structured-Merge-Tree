...
In my diagram earlier,you must have been wondering why the bloom filter was in the read path, the bloom filter is one of the key pieces for read operations in this system.
...
SStables have been mentioned a significant amount of times in this doc.Sorted-String Tables also known as SSTables are files that hold the flushed key-value pairs from the memtable,the inorder traversal method in the memtable makes sure that all data flushed to the sstables must be sorted in ascending order,why?sorted string files enables effienct merging and compaction, it also sets the foundation for index-based reads as a future improvement to the system.
